...
# solution ref: https://www.cnblogs.com/echoT/p/16094953.html
...
class Insect:
    ...
    def remove_from(self, place):
        # Q: why need param of place here?
        self.place = None
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            place.ant.place = self # change contained ant's place to container
            place.ant = self
            Insect.add_to(self, place)
        elif (place.ant.is_container) and place.ant.can_contain(self):
            place.ant.store_ant(self)
            Insect.add_to(self, place.ant)
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        start_place, end_place = self.place, self.place

        if isinstance(self.place, ContainerAnt):
            start_place, end_place = self.place.place, self.place.place
        
        while self.lower_bound > 0:
            if not start_place.is_hive:
        ...
        if self.upper_bound == float('inf'):
            ...
            while self.upper_bound > 0:
                end_place = end_place.entrance
                self.upper_bound -= 1

        def find_bees(place):
            if place.is_hive or (place.entrance is end_place):
                return None
            elif place.bees != []:
                return random_bee(place.bees)
            elif place.bees == []:
                return find_bees(place.entrance)
        
        return find_bees(start_place)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        def fire_attack(damage):
            all_bees = self.place.bees[:]
            for bee_copy in all_bees:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt that does nothing each turn, but has a large health value."""
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt that can eat a random bee in its place and spend 3 turns to chew."""
    ...
    def is_chewing(self):
        return (self.turns_to_chew > 0)
    
    def action(self, gamestate):
        """Eat a random bee in this place and remove it from the place. Then chew it for 3 turns"""
        if self.is_chewing():
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and cause damage to bees"""
    name = 'Tank'
    food_cost = 6
    implemented = True 
    damage = 1
...
class ScubaThrower(ThrowerAnt):
    """A type of ThrowerAnt that is waterproof """
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        def double(place):
            if place.ant and (not place.ant in doubled):
                place.ant.double()
                doubled.append(place.ant)
                if (place.ant.is_container) and (place.ant.ant_contained):
                    place.ant.ant_contained.double()
                    doubled.append(place.ant.ant_contained)
            if place.exit:
                double(place.exit)

        if isinstance(self.place, ContainerAnt):
            double(self.place.place.exit)
        else:
            double(self.place.exit)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        """Throw at the TARGET Bee, making it slow"""
        # BEGIN Problem EC
        def slowed_bee_action(gamestate):
            if target.turns_to_slow and gamestate.time % 2 == 0:
                Bee.action(target, gamestate)
                target.turns_to_slow -= 1
            elif target.turns_to_slow and gamestate.time % 2 != 0:
                target.turns_to_slow -= 1
            else:
                Bee.action(target, gamestate)
        ...
        if target is not None:
            ...
            target.action = slowed_bee_action
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        def add_insect_next_place(place, dist):
            if place:
                if place.ant:
                    insects_and_distances[place.ant] = dist
                if isinstance(place.ant, ContainerAnt) and place.ant.ant_contained:
                    insects_and_distances[place.ant.ant_contained] = dist
                for b in place.bees:
                    insects_and_distances[b] = dist
                dist += 1
                add_insect_next_place(place.entrance, dist)
        if isinstance(self.place, ContainerAnt):
            insects_and_distances[self.place] = 0
            for insect in self.place.place.bees:
                insects_and_distances[insect] = 0
        else:
            for insect in self.place.bees:
                insects_and_distances[insect] = 0
        if self.place.entrance:
            add_insect_next_place(self.place.entrance, 1)
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        self.damage = 2
        self.damage -= 0.25 * distance
        self.damage -= 0.0625 * self.insects_shot
        if self.damage < 0:
            return 0
        return self.damage
        # END Problem Optional 2
    ...
    def action(self, gamestate):
        ...
        print(f'insects_and_distances: {insects_and_distances}')
        for insect, distance in insects_and_distances.items():
            damage = self.calculate_damage(distance)
            print(f'{insect} get damage of: {damage}')
